...
class PreventionCenterEngine {
  ...
  List<PreventionRecommendationItem> _buildVaccines(
    ...
    // dTpa booster — check vaccination dates if available
    {
      ...
      add(
        _item(
          ...
          reason: hasChronicRisk
              ? 'Age 65+ with chronic conditions increases pneumococcal risk'
    ...
    add(
      _item(
        ...
        reason: bundle.profile.takingFolicAcid
            ? 'Folic acid already recorded in the profile'
            : 'Folic acid is commonly recommended before and during early pregnancy',
        actionHint: bundle.profile.takingFolicAcid
            ? 'Confirm the current dosage is appropriate'
            : 'Discuss folic acid timing and dosage with your clinician',
        cadenceLabel: 'Daily during recommended period',
        status: 'recommended',
        priority: 'high',
        category: 'maternal_health',
        kind: 'supplement',
  ...
  List<PreventionRecommendationItem> _buildLungCancerItems(
    ...
    if (age >= policy.lungLdctStartAge && age <= policy.lungLdctEndAge) {
      final isCurrentSmoker = profile.smoker;
      final isFormerSmoker = profile.formerSmoker;
      ...
      if (hasPackYearRisk &&
          (isCurrentSmoker || (isFormerSmoker && quitWithinWindow))) {
        items.add(
          _item(
            ...
            subtitle: null,
            reason:
                'Smoking history and age meet common LDCT screening eligibility criteria',
            actionHint:
      ...
      } else if (isCurrentSmoker || isFormerSmoker) {
        items.add(
          _item(
            ...
            subtitle: null,
            reason:
                'Smoking history is recorded but pack-year data is incomplete for precise risk assessment',
            actionHint:
                'Ask your clinician whether a full smoking history and pack-year calculation should be recorded',
            cadenceLabel: 'At next visit',
            status: 'review',
            priority: 'normal',
            category: 'cancer_screening',
  ...
  List<PreventionRecommendationItem> _buildAaaItems(
    ...
    if (isMale &&
      ...
      items.add(
        ...
        age >= policy.aaaStartAge &&
        age <= policy.aaaEndAge &&
        _containsAnyFamilyHistory(bundle.familyHistory, _aaaRiskTokens)) {
      items.add(
        _item(
  ...
  bool _hasCardiometabolicRisk(ProfileBundle bundle) {
    ...
    return profile.smoker ||
        ...
        _containsAnyCondition(bundle, _pulmonaryRiskTokens);
  }
  ...
  bool _hasColorectalHighRisk(ProfileBundle bundle) {
    return _containsAnyCondition(bundle, _colonHighRiskTokens) ||
        _containsAnyFamilyHistory(bundle.familyHistory, _colonHighRiskTokens);
  }

  bool _hasBreastHighRisk(ProfileBundle bundle) {
    return _containsAnyCondition(bundle, _breastHighRiskTokens) ||
        _containsAnyFamilyHistory(bundle.familyHistory, _breastHighRiskTokens);
  }

  bool _hasFamilyHistoryProstateRisk(ProfileBundle bundle) {
    return _containsAnyFamilyHistory(bundle.familyHistory, const [
      'prostate',
      'prostate cancer',
    ]);
  }
